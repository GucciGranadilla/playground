...
export default function PageTransition({ children }: PageTransitionProps) {
  ...
  useEffect(() => {
    ...
    const initAudio = async () => {
      ...
      const res = await fetch("/effects/transitions.mp3");
    ...
    const handleRouteChangeStart = () => {
      exitPath = router.asPath.split(/[?#]/)[0];
      ...
      const delay = audioCtx.createDelay(12.0);
      ...
      // echo loop: source → delay → wetGain → out
      //                      ↑← feedback ←┘
    ...
    const handleRouteChangeComplete = (url: string) => {
      ...
      const routeChanged = newPath !== exitPath;
